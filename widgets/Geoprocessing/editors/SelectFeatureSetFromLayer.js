  ...
  var clazz = declare([BaseFeatureSetEditor, _TemplatedMixin], {
    ...
    getGPValue: function(){
      if(this.activeViewIndex === 0) {
        ...
        if (layer) {
          var silent = true;
          var doNotQueryServerToGetAllFeatures = true;
          var def2 = this.spatialFilterByFeatures.getFeatureSet(silent, doNotQueryServerToGetAllFeatures);

          def2.then(lang.hitch(this, function(featureSet){
            if (featureSet && featureSet.features && featureSet.features.length > 0) {
              def.resolve(featureSet);
            } else {
              if (this.param.useFeatureCollection) {
                if (!layer.url) {
                  featureSet = jimuUtils.getFeatureSetByLayerAndFeatures(layer, layer.graphics);
                  def.resolve(featureSet);
                } else {
                  var query = new Query();
                  query.where = '1=1';
